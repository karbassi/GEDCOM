"""Starter authoring documents for ``gedcom7 init``.
...
feeding it back into ``gedcom7 build`` produces a valid GEDCOM file.
"""
...
_TEMPLATES = {"yaml": _YAML, "toml": _TOML, "json": _JSON}


def scaffold(fmt: str) -> str:
    """Return a starter authoring document in ``fmt`` (``yaml``/``toml``/``json``)."""
    try:
        return _TEMPLATES[fmt]
    except KeyError:
        raise LoadError(f"unknown init format {fmt!r}; use yaml, toml, or json") from None
