...
The parser is chosen by file extension: ``.json`` and ``.toml`` use the
standard library; ``.yaml``/``.yml`` use PyYAML, imported lazily so it stays
an optional extra (``gedcom7[yaml]``) and the core install needs no runtime
dependency.
"""
...
def read_document(path: str | Path) -> dict[str, Any]:
    ...
    if suffix in _JSON_SUFFIXES:
        ...
        raise LoadError(
            f"unsupported input extension {suffix or '(none)'!r}; use .yaml, .yml, .json, or .toml"
        )
...
def _parse_toml(text: bytes, path: Path) -> Any:
    try:
        return tomllib.loads(text.decode("utf-8"))
    except (tomllib.TOMLDecodeError, UnicodeDecodeError) as error:
        raise LoadError(f"{path}: invalid TOML: {error}") from None
...
def _parse_yaml(text: bytes, path: Path) -> Any:
    try:
        ...
        raise LoadError(
            ...
            "install it with 'pip install gedcom7[yaml]', or use a .json/.toml file"
        ) from None
    try:
